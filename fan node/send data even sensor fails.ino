...
typedef struct {
  uint8_t nodeID;
  float t1;       // temp1 (MAX6675)
  float t2;       // temp2
  float v1;       // RMS1
  float v2;       // RMS2
  float rpm;      // repurposed → DHT temperature
  float voltage;  // repurposed → DHT humidity
} SensorPacket;

SensorPacket packet;

// ============================================================
// SAFE VALUE HANDLER (returns 0 instead of NaN/INF)
// ============================================================
...
void setup() {
  ...
  // ADXL init (but do not stop program if one fails)
...
void loop() {
  ...
  if (now - lastSample >= (1000 / SAMPLE_RATE)) {
    ...
    float mag1 = safeValue(
      sqrt(e1.acceleration.x * e1.acceleration.x +
           e1.acceleration.y * e1.acceleration.y +
           e1.acceleration.z * e1.acceleration.z)
    );

    float mag2 = safeValue(
      sqrt(e2.acceleration.x * e2.acceleration.x +
